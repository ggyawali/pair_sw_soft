//Soft core SW potential by Gaurav Gyawali and Steven W Rick
...
PairSWSoft::PairSWSoft(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  one_coeff = 1;
...
void PairSWSoft::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if ((itype == type_solvent && jtype != type_solvent) || (itype !=type_solvent && jtype == type_solvent))
	{
	 muij = mu[1][1];
	 //cout << muij;
	}
	...
	muij = 1;
      ...
      twobody(&params[ijparam],rsq,fpair,eflag,evdwl, muij);
...
void PairSWSoft::settings(int narg, char **arg)
{
  if (narg != 4) error->all(FLERR,"Illegal pair_style command");
  mu_one = force->numeric(FLERR,arg[0]);
...
void PairSWSoft::coeff(int narg, char **arg)
{
  int i,j,n;
...
//mu_one = force->numeric(FLERR,arg[2]);
  if (elements) {
...
double PairSWSoft::init_one(int i, int j)
{
...
void PairSWSoft::read_file(char *file)
{
...
void PairSWSoft::twobody(Param *param, double rsq, double &fforce,
                     int eflag, double &eng, double mup)
{
//double sfforce,sr,srinvsq,srp,srq,srainv,srainvsq,sexpsrainv;

//sr = sqrt(rsq);
//srinvsq = 1.0/rsq;
//srp = pow(sr,-param->powerp);
//srq = pow(sr,-param->powerq);
//srainv = 1.0 / (sr - param->cut);
//srainvsq = srainv*srainv*sr;
//sexpsrainv = exp(param->sigma * srainv);
//fforce = (param->c1*srp - param->c2*srq +
//          (param->c3*srp -param->c4*srq) * srainvsq) * sexpsrainv * srinvsq;
//if (eflag) eng = (param->c5*srp - param->c6*srq) * sexpsrainv;
//out<<"Force SW "<<fforce << eng <<endl;

   double r, rainv, rinvsq, rbysig, rbysigp, rbysigq, rainvsq, expo,denomp,denomq, coef;
   
   //printf("%f\n", mup);
   //double powern = powern;
   r = sqrt(rsq);
   rinvsq = 1.0/rsq;
   rbysig = r/param->sigma;
   rbysigp = pow(rbysig, param->powerp);
   rbysigq = pow(rbysig, param->powerq);
   rainv = 1.0 / (r - param->cut);
   rainvsq = rainv*rainv;
   expo = exp(param->sigma * rainv);
   denomp = alpha * (1-mup)*(1-mup) + rbysigp;
   denomq = alpha * (1-mup)*(1-mup) + rbysigq;
   coef = param->epsilon * pow(mup, powern) * rinvsq * param->biga * expo;
    fforce = coef * (param->bigb * param->powerp * rbysigp/(denomp*denomp) -                          	param->powerq * rbysigq/(denomq*denomq)  + param->sigma * rainvsq * r * ( param->bigb/denomp - 1/denomq));
 
 
   if (eflag) eng = pow(mup,powern)*param->biga * param->epsilon*expo*(param->bigb/denomp - 1/denomq); 
//    cout<<"Force Soft Core SW " << fforce << eng<< " "<< mup <<endl; 
}
...
/////
void *PairSWSoft::extract(const char *str, int &dim)
{
  dim = 2;
